...
HAL_StatusTypeDef MX_Queue_Config(void)
{
  ...
  /* Build node_config Node */
  ret |= HAL_DMAEx_List_BuildNode(&pNodeConfig, &node_config);

  /* Insert node_config to Queue */
  ret |= HAL_DMAEx_List_InsertNode_Tail(&Queue, &node_config);
