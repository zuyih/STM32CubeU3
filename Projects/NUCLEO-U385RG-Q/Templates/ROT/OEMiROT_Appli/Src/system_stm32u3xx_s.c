...
void SecureSystemInit(void)
{
  ...
  /* Clock enabling */
  RCC->AHB1ENR1 |= 0x81031109UL;
  RCC->AHB1ENR2 |= 0x00000004UL;
  RCC->AHB2ENR1 |= 0x483F0CDFUL;
  RCC->AHB2ENR2 |= 0x00000010UL;
  RCC->APB1ENR1 |= 0x71FC4137UL;
  RCC->APB1ENR2 |= 0x00000220UL;
  RCC->APB2ENR  |= 0x09275800UL;
  RCC->APB3ENR  |= 0x0000B8C2UL;

  /* Illegal access interrupts configuration */
  GTZC_TZIC1->IER1 |= 0x01E6FDF7UL;
  GTZC_TZIC1->IER2 |= 0x0001FEFBUL;
  GTZC_TZIC1->IER3 |= 0x01D3F958UL;
  GTZC_TZIC1->IER4 |= 0x0F00C1FFUL;

  /* Securable peripherals security and privilege management */
  GTZC_TZSC1->SECCFGR1  |= 0x01E6FDF7UL;
  GTZC_TZSC1->SECCFGR2  |= 0x0001FEFBUL;
  GTZC_TZSC1->SECCFGR3  |= 0x01D3F958UL;

  /* TrustZone-aware peripherals security and privilege management */

  /* PWR */
  PWR->SECCFGR  |= 0x0000F3FFUL;

  /* RCC */
  RCC->SECCFGR  |= 0x00001CFFUL;
  ...
  GPIOE->SECCFGR |= 0x0000FFFFUL;
  GPIOG->SECCFGR |= 0x0000FFFCUL;
  GPIOH->SECCFGR |= 0x0000000BUL;

  /* SYSCFG */
  SYSCFG->SECCFGR |= 0x0000000B;

  /* DMA */
  GPDMA1->SECCFGR  |= 0x00000FFFUL;

  /* EXTI */
  EXTI->SECCFGR1  |= 0x0007FFFFFUL;
  ...
  PWR->DBPR     &= ~PWR_DBPR_DBP;
   ...
   /* Clock disabling */
   RCC->AHB1ENR1 &= ~(0x00031009UL);
   RCC->AHB1ENR2 &= ~(0x00000004UL);
   RCC->AHB2ENR1 &= ~(0x083F0CDFUL);
   RCC->AHB2ENR2 &= ~(0x00000010UL);
   RCC->APB1ENR1 &= ~(0x71FC4137UL);
   RCC->APB1ENR2 &= ~(0x00000220UL);
   RCC->APB2ENR  &= ~(0x09275800UL);
